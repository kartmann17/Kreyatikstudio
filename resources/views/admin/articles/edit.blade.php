    ...
    <div class="bg-white rounded-xl shadow-lg">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-t-xl px-6 py-4">
            <h3 class="text-lg font-semibold flex items-center mb-2 sm:mb-0">
                ...
                Modification: {{ Str::limit($article->title, 40) }}
            </h3>
            <a href="{{ route('admin.articles.index') }}" class="bg-white text-blue-600 hover:bg-blue-50 px-4 py-2 rounded-lg font-semibold flex items-center transition-all duration-200">
        ...
        <div class="p-6">
            ...
            <form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data" id="articleForm">
                        ...
                        <div class="bg-gray-50 rounded-lg p-4">
                            <div class="flex items-center justify-between">
                                ...
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" id="is_published" name="is_published" value="1" {{ old('is_published', $article->is_published) ? 'checked' : '' }} class="sr-only peer">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="relative inline-block">
                                <img src="{{ Storage::url($article->image) }}" alt="{{ $article->title }}" 
                                     class="w-full h-64 object-cover rounded-lg shadow-sm" id="currentImage">
                                <div class="mt-3">
                                    <label class="flex items-center cursor-pointer">
                                        <input type="checkbox" id="remove_image" name="remove_image" class="form-checkbox h-4 w-4 text-red-600 transition duration-150 ease-in-out">
                                        <span class="ml-2 text-sm text-red-600 hover:text-red-800">
                        ...
                        <div>
                            <label for="image" class="block text-sm font-medium text-gray-700 mb-2">
                                {{ $article->image ? 'Nouvelle image (optionnel)' : 'Image de l\'article' }}
                            </label>
                            <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 transition-colors duration-200" id="dropZone">
                                <input type="file" id="image" name="image" accept="image/*" class="hidden">
                                <div id="uploadArea">
                                    <i class="fas fa-cloud-upload-alt text-4xl text-gray-400 mb-4"></i>
                                    <p class="text-gray-600 mb-2">Cliquez pour sélectionner une nouvelle image</p>
                        ...
                        <div id="preview-container" class="hidden">
                            ...
                            <div class="relative">
                                ...
                                <button type="button" id="removeNewImage" class="absolute top-2 right-2 bg-red-500 hover:bg-red-600 text-white p-2 rounded-full transition-colors duration-200">
                        ...
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                            <div class="flex items-start">
                                ...
                                <div>
                                    <h4 class="text-sm font-semibold text-blue-800">Informations article</h4>
                                    <div class="text-sm text-blue-700 mt-2 space-y-1">
                                        <p><strong>Créé le :</strong> {{ $article->created_at->format('d/m/Y à H:i') }}</p>
                                        <p><strong>Modifié le :</strong> {{ $article->updated_at->format('d/m/Y à H:i') }}</p>
                                        @if($article->published_at)
                                        <p><strong>Publié le :</strong> {{ $article->published_at->format('d/m/Y à H:i') }}</p>
                                        @endif
                                    </div>
                                </div>
                            </div>
                ...
                <div class="flex justify-end pt-6 border-t border-gray-200 mt-8">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-lg font-semibold flex items-center transition-all duration-200 shadow-lg hover:shadow-xl">
    ...
    function handleImageSelection(file) {
        ...
        if (!file.type.match('image.*')) {
            Swal.fire({
                ...
                icon: 'error',
                confirmButtonClass: 'bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg'
            });
            return;
        }
