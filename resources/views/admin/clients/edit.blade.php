...
@section('content_body')
    <div class="bg-white rounded-lg shadow-sm">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex justify-between items-center">
                <h3 class="text-lg font-semibold text-gray-900 flex items-center">
                    <i class="fas fa-user-edit mr-2 text-blue-600"></i>Modifier les informations du client
                </h3>
        ...
        <form action="{{ route('admin.clients.update', $client->id ?? 1) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('notes') border-red-500 focus:ring-red-500 focus:border-red-500 @enderror" 
                              ...
                              name="notes" 
                              rows="3" 
                              placeholder="Notes supplémentaires...">{{ old('notes', $client->notes ?? '') }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
